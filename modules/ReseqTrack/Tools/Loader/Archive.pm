...
sub new {
 ...
 $self->create_archive_action_adaptor();
 $self->create_archive_location_adaptor();
 $self->create_archive_actions();
 ...
 #  $self->initial_table_clean(); # automatic in original script.
...
sub check_sanity {
 ...
 print Dumper (@files_to_archive) if $self->debug;
...
sub location_root {
 my ( $self, $arg ) = @_;
 $self->{location_root} = $arg if ( defined $arg );
...
sub other_location {
 my ( $self, $arg ) = @_;
 $self->{archive_location} = $arg if ( defined $arg );
...
sub archive_actions {
 my ( $self, $arg ) = @_;
 $self->{archive_actions} = $arg if ( defined $arg );
...
sub path_like {
 my ( $self, $arg ) = @_;
 $self->{path_like} = $arg if ( defined $arg );
 return $self->{path_like};
}

sub action_string {
 my ( $self, $arg ) = @_;
 $self->{action_string} = $arg if ( defined $arg );
...
sub action_location_name {
 my ( $self, $arg ) = @_;
 $self->{action_location_name} = $arg if ( defined $arg );
 return $self->{action_location_name};
}

sub other_location_name {
 my ( $self, $arg ) = @_;
 $self->{other_location_name} = $arg if ( defined $arg );
...
sub from_db {
 my ( $self, $arg ) = @_;
 $self->{from_db} = $arg if ( defined $arg );
 return $self->{from_db};
}

sub priority {
 my ( $self, $arg ) = @_;
 $self->{priority} = $arg if ( defined $arg );
}
