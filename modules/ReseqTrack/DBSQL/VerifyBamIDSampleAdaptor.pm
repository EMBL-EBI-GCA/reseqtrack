...
sub store {
   ...
   # print Dumper $verifybamid_Sample;
	...
	my $sql =
	  ...
	  . "num_run_ids, num_low_selfIBD_run_ids, failed,performed ) "
...
sub update {
	...
	my $sql =
	  ...
	  . " num_low_selfIBD_run_ids = ?, failed = ? , performed = now()"
	  . " where other_id  = ? and table_name = ? ";
    ...
    $sth->bind_param( 15, $verifybamid_Sample->other_id );
    $sth->bind_param( 16, $verifybamid_Sample->table_name );
