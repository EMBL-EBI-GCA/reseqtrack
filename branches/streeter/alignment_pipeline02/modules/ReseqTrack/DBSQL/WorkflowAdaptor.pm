...
sub fetch_by_goal_event_id{
  ...
  my $sql = "select ".$self->columns." from ".$self->table_name.
      " where workflow_goal.goal_event_id = ?";
...
sub store{
  ...
  if(!$dbID){
    my $existing_goal = $self->fetch_by_goal_event($workflow->goal_event);
    $dbID = $existing_goal->dbID;
  }
  $workflow->dbID($dbID);
  ...
  $goal_sth->finish;
  $self->add_condition($workflow, $workflow->conditions);
}
