...
sub run {
  ...
  $self->set_ncbi36snp_info_from_build if ($build eq "ncbi36");
  $self->set_grc37snp_info_from_build  if ($build eq "grc37");

  $self->process_bam_sam if ( $self->sam() || $self->bam() );
...
sub input_sanity_check {
  ...
  throw "No reference sequence specified" if ( !$self->reference && ! $self->build );
...
sub snp_inventory {
  my ( $self, $arg ) = @_;
  if ($arg) {
    $self->{snp_inventory} = $arg;
  }
  return $self->{snp_inventory};
}
sub reference {
  my ( $self, $arg ) = @_;
  if ($arg) {
