# EnsEMBL module for ReseqTrack::Tools::Exception
#
#

=head1 NAME

ReseqTrack::Tools::Exception - Utility functions for error handling

=head1 SYNOPSIS

    use ReseqTrack::Tools::Exception qw(throw warning deprecate verbose try catch);
...
package ReseqTrack::Tools::Exception;
...
=head2 throw
  ...
  Example    : use ReseqTrack::Tools::Exception qw(throw);
...
sub throw {
  ...
  $string = shift if($string eq "ReseqTrack::Tools::Exception");
...
=head2 warning
  ...
  Example    : use ReseqTrack::Tools::Exception qw(warning)
               warning('This is a warning');
...
sub warning {
  my $string = shift;
  $string = shift if($string eq "ReseqTrack::Tools::Exception"); #skip object if one provided
  my $level  = shift;
...
=head2 info
  ...
  Example    : use ReseqTrack::Tools::Exception qw(verbose info)
...
=head2 verbose
  ...
  Arg [1]    : (optional) int 
  Example    : use ReseqTrack::Tools::Exception qw(verbose warning);
...
sub verbose {
  if(@_) {
    my $verbosity = shift;
    $verbosity = shift if($verbosity eq "ReseqTrack::Tools::Exception");
...
=head2 stack_trace_dump
  ...
  Example    : use ReseqTrack::Tools::Exception qw(stack_trace_dump);
...
sub stack_trace_dump{
  ...
  $levels = shift if(@_);
  $levels = shift if($levels eq "ReseqTrack::Tools::Exception");
...
=head2 stack_trace
  ...
  Arg [1]    : none
  Example    : use ReseqTrack::Tools::Exception qw(stack_trace)
...
=head2 deprecate
  ...
  Example    : use ReseqTrack::Tools::Exception qw(deprecate)
               sub old_sub {
...
sub deprecate {
  my $mesg = shift;
  $mesg = shift if($mesg eq "ReseqTrack::Tools::Exception"); #skip object if one provided
...
=head2 try/catch
  ...
  Example    : use ReseqTrack::Tools::Exception qw(throw try catch)
               The syntax is:
