...
use ReseqTrack::Tools::FileSystemUtils
  qw (create_tmp_process_dir delete_directory);
...
GetOptions(
	...
	'snps_list=s',   'vcf=s',	'name=s',  'collection_type=s',
...
my $have_result = $va->fetch_by_file_id_and_readgroup( $sample_self->file_id, $sample_self->read_group);
$have_result ?  $va->update (  $sample_self ) :  $va->store(  $sample_self ) unless ($input{test});


foreach my $rg (@$read_group){
  my $have_result = $va->fetch_by_file_id_and_readgroup( $rg->file_id, $rg->read_group);
...
sub set_options{
	...
	if ( $input{bam_name} =~ /exome/i )
	  {
	    $input{options} = " --maxDepth 1000 --precise ";
	  }	
...
sub set_analysis_mode {
	  ...
	  if ( !defined $hash{$sample} ) {
		  ...
		  $$input{analysis_type} = "FREE";
                  $$input{options} .= " --chip_none "; #turn off chip_mix calculation
