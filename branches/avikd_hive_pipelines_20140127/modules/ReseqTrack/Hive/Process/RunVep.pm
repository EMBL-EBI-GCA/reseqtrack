...
sub run {
    ...
    my $vcf = $self->param_as_array('vcf');
...
#     my $bed = $$beds[0];
    ...
    my $vep_filter = $self->param_is_defined('vep_filter') ? $self->param('vep_filter') : 0;
    my $vep_filter_exe = $self->param_is_defined('vep_filter_exe') ? $self->param('vep_filter_exe') : undef;
    my $vep_filter_options = $self->param_is_defined('vep_filter_options') ? $self->param('vep_filter_options') : undef;
...
#########
throw("expected exactly one slice, not ". scalar @$slices." ".$SQ_start." ".$bp_start." ".$bp_end) if scalar @$slices != 1;
...
my $vep_runner = ReseqTrack::Tools::RunVep->new(
      ...
      -vep_filter => $vep_filter,
      -vep_filter_exe => $vep_filter_exe,
      -vep_filter_options => $vep_filter_options,
  ...
  #  $self->output_param('bp_start' => $bp_start);
  #  $self->output_param('bp_end' => $bp_end );
