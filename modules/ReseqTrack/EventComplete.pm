...
sub new{
  ...
  my ($event, $event_id, $other, $other_id, $type, $table_name, $success, $time,
      $other_name) = rearrange([qw(EVENT EVENT_ID OTHER OTHER_ID TYPE TABLE_NAME 
                                   SUCCESS TIME OTHER_NAME)], @args);
  #error checking
  ...
  throw("Must pass EventComplete a table name and it must be file ".
        "or collection or run_meta_info") if(!$table_name || 
                             ($table_name ne 'file' && 
...
sub event{
  my ($self, $arg) = @_;
  if($arg){
    throw("Must pass ReseqTrack::EventComplete::event an event object not ".$arg)
...
sub event_id{
  ...
  if($self->adaptor){
    if(!$self->event){
      ...
      my $event = $ea->fetch_by_dbID($arg);
      throw("Failed to fetch an event with dbID $arg") if($arg && !$event);
      $self->event($event);
    }
  }
...
sub other_id{
  ...
  if($id){
    ...
    if($self->adaptor && !$self->other){
      ...
      warning("Failed to fetch ".$self->table_name." using id ".$id) if(!$other);
      $self->{other} = $other;
...
sub get_other{
  ...
  }elsif($self->table_name eq 'collection' || $self->table_name eq 'input_string'){
    return $self->get_other_adaptor->fetch_by_name_and_type($name, $event->type);
  }else{
...
sub get_other_adaptor{
  ...
  if($self->table_name eq "file"){
    ...
    throw("Don't know what sort of adaptor to get for ".$self->table_name);
  }
}
