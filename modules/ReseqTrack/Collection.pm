...
sub new{
  ...
  throw("Can't create a ReseqTrack::Collection object without a name") unless($name);
  throw("Can't create a ReseqTrack::Collection object without a type") unless($type);
  throw("Can't create a ReseqTrack::Collection object wihtout a table_name") unless($table_name);
  if($table_name){
    throw("ReqseqTrack::Collection::table_name must be file, alignment_meta_info ".
          "or run_meta_info not ".$table_name) unless($table_name eq 'file' || 
  ...
  unless($self->sanity_checked_others){
    ...
    if($others && @$others >= 2){
      ...
      foreach my $object(@$others){
        throw("ReseqTrack::Collection ".$object." needs to be ".$first." not ".
              "what it is ") unless($first eq ref($object));      
              }
    }
