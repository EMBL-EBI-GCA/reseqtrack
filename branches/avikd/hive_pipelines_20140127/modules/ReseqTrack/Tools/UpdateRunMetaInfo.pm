...
sub new {
	...
	bless $self, $class;
	my (
		$era_db,              $dcc_db,   $era_dbuser,
		$era_dbpass,          $dbname,   $dbuser,
		$dbport,              $dbpass,   $dbhost,
...
sub population_rules {
	...
	if ( !$self->{population_rules} || $force_update ) {
		$self->{population_rules} =
			$self->dcc_db->get_PopulationRuleAdaptor->fetch_all_in_order();
	}
...
sub find_differences {
	...
	my ( $era_only, $dcc_only, $diff ) =
		compare_era_and_dcc_meta_info( $era_rmis, $dcc_rmis );

	#if($self->verbose){
	print $logging_fh "There are " .
		keys(%$era_only) . " runs only in the ERA " . "database\n";
	print $logging_fh "There are " .
		keys(%$dcc_only) . " runs only in the DCC " . "database\n";
	print $logging_fh "There are " .
		keys(%$diff)
		. " differences in runs in both "
...
sub store_new_entries {
	...
	foreach my $run_id ( keys(%$rmis) ) {
		...
		eval { $rmia->store($rmi); };
		if ($@) {
			print $logging_fh "Failed to store "
				. $self->rmi_summary_string($rmi) . "\n";
...
sub update_collections {
	...
	foreach my $name ( keys(%$study_id_hash) ) {
		...
		unless ($collection) {
			...
			my $file_set =
				ReseqTrack::Tools::Intersection->new( -list => \@run_ids, );
			my $db_set =
				ReseqTrack::Tools::Intersection->new( -list => \@collection_run_ids, );
	...
	if ( !$self->{study_id_hash} || $reparse ) {
		...
		while (<FH>) {
			...
			if (/^\[(.*)\]\s*$/) {    # $1 will be the header name, without the []
				...
				if ( $config{$header} ) {
					throw(
						"Shouldn't have duplicate headers in " . $file . " " . $header );
				}
				$config{$header} = {};
			}
			else {
...
sub find_sample_diffs {
	...
	foreach my $run_id ( keys( %{ $self->diff_set } ) ) {
		...
		unless ( $era_rmi->sample_name eq $dcc_rmi->sample_name ) {
			...
			print $logging_fh $run_id
				. " has a sample difference "
				. $dcc_rmi->sample_name
				. " has become "
				. $era_rmi->sample_name
				. " in the "
...
sub check_unidentified {
	...
	print $logging_fh "There are "
		. @era_problems
		. " era runs with unidentified samples\n"
		if (@era_problems);
	print "There are " . @dcc_problems . " dcc runs with unidentified samples\n"
		if (@dcc_problems);
	print $logging_fh "There are "
		. @dcc_problems
		. " dcc runs with unidentified samples\n"
		if (@dcc_problems);

	foreach my $rmi (@era_problems) {
		print $logging_fh $rmi->run_id . " "
			. $rmi->center_name . " "
			. $rmi->study_name . " "
			. $rmi->sample_name . " "
			. $rmi->population . "\n";
