...
sub list_files_in_dir{
  ...
  throw("Must pass list_files_in_dir a directory not ".$dir) unless( -d $dir);
...
sub get_md5hash{
  ...
  my %md5hash;  while(<FH>){
    ...
    throw($filename." has zero md5 in ".$file) if(!$md5 || ($md5 =~ /^\d+$/ && $md5 == 0));
    warning($filename." already existing in hash") if($filehash{$filename});
  ...
  while(<FH>){
    ...
    if($hash{$values[$key_column]}){
      warning("ReseqTrack::Tools::FileUtils::get_columns_from_tab_file ".
              "assumes that the contents of ".$key_column." in ".$file." are ".
              "unique but they appear not to be, this will means ".$value_column.
