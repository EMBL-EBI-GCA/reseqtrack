...
sub columns {
  return
"sample.sample_id, cv_status.status, sample.md5, sample.center_name, sample.sample_alias, sample.tax_id, sample.scientific_name, sample.common_name, sample.anonymized_name, sample.individual_name, sample.sample_title, submission.submission_id, to_char(submission.submission_date, 'YYYY-MM-DD HH24:MI') submission_date, sample.ega_id ";
}

sub where {
  return "sample.status_id = cv_status.status_id and sample.submission_id = submission.submission_id";
}
