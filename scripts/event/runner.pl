...
$db->dbc->disconnect_when_inactive(1);
 JOB:foreach my $job(@jobs){
   ...
   if(!$analysis){
     job_failed($job, "Can't run ".$job." ".$job->dbID." ".
                $job->input_string." without an analysis", $ja);
   }
   ...
   eval{
     $ca->store($completed_string) if($job->current_status eq 'SUCCESSFUL');
   };
   if($@){
     my $warning = "Failed to store ".$input_string." in completed_string $@";
   ...
   if($job->current_status eq 'SUCCESSFUL'){
     ...
     throw("Failed to remove ".$job." from ".$dbname." $@") if($@);
   }
}
