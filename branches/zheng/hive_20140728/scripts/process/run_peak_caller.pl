...
if ($preserve_paths) {
    ...
    $job_name   = $filename;
    $job_name =~ s/\.bam$//;
    $job_name =~ s/bwa_filtered/macs2/ if ( $module_name =~ m/Macs2/ );
}
else {
    $output_dir =
...
print STDOUT join( "\t", "Using control:", @$control_filepaths ) . $/
  if $control_filepaths;
...
if ($do_peak_stats) {
    ...
    while ( my ( $key, $value ) = each %peak_stats ) {
        push @stats,
          create_attribute_for_object( $output_collection, $key, $value )
          if ($key);
    }
