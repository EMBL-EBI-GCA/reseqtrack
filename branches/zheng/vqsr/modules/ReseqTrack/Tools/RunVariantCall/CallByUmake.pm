...
sub new {
  ...
  ## Set defaults	 
	$self->{'options'}->{'offset_off_target'} = 0 unless ($offset_off_target); 
	...
	$self->{'options'}->{'FILTER_MIN_SAMPLE_DP'} = 0.5 unless ($FILTER_MIN_SAMPLE_DP);
 	$self->{'options'}->{'FILTER_MQ'} = 3 unless ($FILTER_MQ);
 	$self->{'options'}->{'FILTER_FLAG'} = '0x0704' unless ($FILTER_FLAG);
	$self->{'options'}->{'unit_chunk'} = 5000000 if (!$unit_chunk);
  	$self->{'options'}->{'LD_Nsnps'} = 10000 if (!$LD_Nsnps);
  	$self->{'options'}->{'LD_overlap'} = 1000 if (!$LD_overlap);
	...
	my $root_dir = $self->working_dir;
	`mkdir $root_dir` unless ( -e $self->working_dir);
	
	my $outdir = $self->working_dir . "/" . $self->chrom . "." . $self->region; ##specify chrom_chunk in output dir root so it can distinguish vcfs generated by different jobs 
	
	eval {
		if (-e $outdir) {
			warn("Output dir $outdir exisits. Remove the content and re-build it now .....\n"); 
			`rm -r -f $outdir`;
		}
	};
	throw("removing exisitng output dir $outdir failed, $@\n") if $@;
	
	eval {
		`mkdir $outdir`;
	};
	throw("mkdir failed for output dir $outdir, $@\n") if $@;
		 
	$self->umake_output_dir($outdir);
