...
sub new {
 ...
 $self->build( "grc37");
 $self->program( '/nfs/1000g-work/G1K/work/bin/bfast-0.6.4d/bin/bfast') if (! $self->program);
 $self->preprocess_exe('/nfs/1000g-work/G1K/work/bin/bfast-0.6.4d/bin/1kg2bfastfastq/1kg_2_bfast_fastq') if (! $preprocess_exe);
...
sub run{
  ...
  print "Working in ", $self->working_dir(), "\n";
   ...
   #process G1K fastq via  
    $exit = system( $self->fastq_cmd );
    if($exit && $exit >= 1){
      throw("Failed to run ". $self->fastq_cmd);
    }
