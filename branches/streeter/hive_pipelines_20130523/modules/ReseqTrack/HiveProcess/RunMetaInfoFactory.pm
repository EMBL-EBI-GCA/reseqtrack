...
sub run {
    ...
    $self->param_required('type_branch');
    my $type_branch = $self->param('type_branch');
    ...
    if (lc($type_branch) eq 'sample') {
      ...
      while (my $row = $sth->fetchrow_arrayref) {
        ...
        $self->prepare_child_output_id($sample_name, {'sample_id' => $sample_id, 'sample_name' => $sample_name});
      }
    }
      ...
      foreach my $library_name (map {$_->[0]} @{$sth->fetchall_arrayref()}) {
        $self->prepare_child_output_id($library_name, {'library_name' => $library_name});
      }
    }
      ...
      foreach my $run_id (map {$_->[0]} @{$sth->fetchall_arrayref()}) {
        $self->prepare_child_output_id($run_id, {'run_id' => $run_id});
      }
    }
