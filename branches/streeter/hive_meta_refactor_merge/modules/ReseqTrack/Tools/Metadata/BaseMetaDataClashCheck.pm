  ...
  while ( my $rs = $sth->fetchrow_arrayref ) {
    ...
    my $is_clash =
      ( !exists $white_list->{$col_table_name}->{$column_name}
        ->{$attr_table_name}->{$attribute_name} );
    ...
    if ($is_clash) {
      print join( "\t",
        $col_table_name, $column_name, $attr_table_name, $attribute_name )
        . $/;
      $clashes_count++;
    }
  }
...
sub _sql_attribute_2_attribute_clash {
  return <<'END_SQL';
select distinct a.attribute_name, least(a.table_name,b.table_name),greatest(a.table_name,b.table_name) from 
...
sub _sql_attribute_2_column_clash {
  return <<'END_SQL';
select distinct c.column_name, c.table_name, a.attribute_name, a.table_name from
