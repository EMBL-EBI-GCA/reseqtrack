=head1 NAME
 ...
 ReseqTrack::Hive::PipeConfig::Rna_Alignment_conf
...
package ReseqTrack::Hive::PipeConfig::Rna_Alignment_conf;
...
sub default_options {
    ...
    return {
        ...
        'gsnap' => undef,
        'star'  => '/nfs/1000g-work/G1K/work/bin/STAR/STAR',
        'known_snps' => undef,
        'known_splice_junction' => undef,
        
        'reference' => undef,
        'ref_flat' => undef,
        'star_ribosomal_intervals' => undef,
        ...
        'bai_type' => 'RNA_BAI',
        'bas_type' => 'RNA_BAS',
        'matrics_type' => 'RNASEQ_MATRICS',
        'validate_bam_exe' => '/nfs/1000g-work/G1K/work/avikd/ehive/reseqtrack/avikd/hive_pipelines_20140127/c_code/validate_bam/validate_bam',

        
        'gsnap_options' => { 'thread' => 4},
        ...
        name_file_params => {
            ...
            new_basename => '#run_source_id#.star',
            add_datestamp => 1,
            suffix => '.bam',
          },
...
sub resource_classes {
    ...
    return {
            ...
            '20Gbt4' => { 'LSF' => '-C0 -M20000 -n 4 -q '.$self->o('lsf_queue').' -R"select[mem>20000] rusage[mem=20000] span[hosts=1]"' },
            '30Gb' => { 'LSF' => '-C0 -M30000 -q '.$self->o('lsf_queue').' -R"select[mem>30000] rusage[mem=30000]"' },
            '30Gbt4' => { 'LSF' => '-C0 -M30000 -n 4 -q '.$self->o('lsf_queue').' -R"select[mem>30000] rusage[mem=30000] span[hosts=1]"' },
    };
}
...
sub pipeline_analyses {
    ...
    push(@analyses, {
           -logic_name => 'star_alignment',
            -module        => 'ReseqTrack::Hive::Process::RunRnaAlignment',
            -parameters    => {
                module_name => 'STAR',
                program_file => $self->o('star'),
                output_format => 'BAM',
                samtools => $self->o('samtools_exe'),
                reference => $self->o('reference'),
                options => $self->o('star_options'),
                RGSM => $self->o('RGSM'),
                RGPU => $self->o('RGPU'),
            },
            -rc_name => '30Gbt4', 
    ...
    push(@analyses, {
            ...
            -flow_into => {1 => {'rna_matrics' => {'bas' => '#file#'}}},
      });
      push(@analyses, {
          -logic_name => 'rna_matrics',
          -module        => 'ReseqTrack::Hive::Process::RunPicard',
          -parameters => {
              picard_dir => $self->o('picard_dir'),
              jvm_args => '-Xmx2g',
              command => 'rna_seq_metrics',
              options => {
                ref_flat => $self->o('ref_flat'),
                ribosomal_intervals => $self->o('star_ribosomal_intervals'),
                reference_sequence => $self->o('reference_sequence'),
                strand_specificity => $self->o('picard_strand_specificity'),
              },
          },
          -rc_name => '4Gb',
          -hive_capacity  =>  50,
          -flow_into => {1 => ['store_matrics']},
    });
    push(@analyses, {
            -logic_name    => 'store_matrics',
            -module        => 'ReseqTrack::Hive::Process::LoadFile',
            -parameters    => {
              type => $self->o('matrics_type'),
              file => '#matrics#',
              name_file_module => $self->o('name_file_module'),
              name_file_method => $self->o('name_file_method'),
              name_file_params => {new_full_path => '#bam#.rnaseq_matrics'},
            },
            -rc_name => '200Mb',
            -hive_capacity  =>  200,
