...
sub run_program {
	...
	push @cmd_args, '-p', $self->options('threads') if ($self->options('threads'));
	push @cmd_args, '--GTF', $self->transcript_annotation if ($self->transcript_annotation && $self->options('quantification'));
	push @cmd_args, '--GTF-guide', $self->transcript_annotation if ($self->transcript_annotation && $self->options('guided_assembly'));
	push @cmd_args, '--mask-file', $self->options('mask_file') if ($self->options('mask_file'));
	push @cmd_args, '--frag-bias-correct' , $self->options('bias_correction_fasta') if ($self->options('bias_correction_fasta'));
	push @cmd_args, '--multi-read-correct' if ($self->options('multi_read_correct'));
	push @cmd_args, '--library_type', $self->library_type if ($self->library_type);
	
	# Advanced Abundance Estimation Options:
	for my $param_opt ( qw(frag-len-mean frag-len-std-dev max-mle-iterations max-bundle-frags ) ){
		push @cmd_args, "--$param_opt", $self->options($param_opt) if (defined $self->options($param_opt));
	}
	
	for my $binary_opt ( qw(upper_quartile_norm total_hits_norm compatible_hits_norm no-effective-length-correction) ){
...
sub transcript_annotation {
	my $self = shift;
	  if (@_) {
	    $self->{'transcript_annotation'} = (shift) ? 1 : 0;
	  }
