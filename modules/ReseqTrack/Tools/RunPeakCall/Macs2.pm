...
sub DEFAULT_OPTIONS {
  return {
    ...
    nomodel =>
      0,    #--nomodel #	While on, MACS will bypass building the shifting model.
    shiftsize =>
      undef
...
sub run_program {
  ...
  my $file_prefix = $job_name . '_macs2';
  ...
  if ( $self->fragment_size ) {
    ...
    print join(' ','Fragment size',$self->fragment_size,'Shift size',$shift_size,$/);
...
#execute_system_command($cmd); # this failed silently if run with $self->execute_command_line
  ...
  unshift @output_suffixes, '_broad_peaks.bed' if ( $self->options('broad') );
  my $bed_file_suffix = $output_suffixes[0];
